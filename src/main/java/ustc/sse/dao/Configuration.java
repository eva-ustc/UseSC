...
public class Configuration {
    ...
    /**
     * 构造函数里初始化table,读取or_mapping信息到table
     */
...
//            Document document = reader.read(new FileInputStream("D:/or_mapping.xml")); //测试环境映射文件
    ...
    public  String getTablePK(){
        return table.get("id").get("column");
    }
    ...
    public  String getTableColumn(String entity_attr){
        return table.get(entity_attr).get("column");
    }
    ...
    public String getTableName() {
        return table.get("table").get("table");
    }
    ...
    public String getEntityName(){
        return table.get("name").get("name");
    }
    ...
    public  Boolean isLazyLoad(String entity_attr){
        ...
        Map<String, String> map = table.get(entity_attr);
        if (map != null ) {
            switch (map.get("lazy")){
                case "true":
                    return true;
